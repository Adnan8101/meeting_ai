...
export default function RadialOrbitalTimeline({
  ...
  return (
    <div
      className={`relative w-full ${heightClassName} flex flex-col md:flex-row items-center justify-center overflow-hidden bg-black`}
    >
      {/* ── Orbit stage container ─────────────────────────────────── */}
      <div
        className="relative flex items-center justify-center shrink-0"
        style={{
          width: hasSelection && !isMobile ? '50%' : '100%',
          height: isMobile && hasSelection ? '45%' : '100%',
      ...
      >
        <div
          ...
          style={{
            width: diameter + 100,
            height: diameter + 100,
            transform: hasSelection && !isMobile ? 'scale(0.85)' : 'scale(1)',
      ...
      {/* ── Detail panel (slides in from right / bottom) ──────────── */}
      <div
        style={{
          width: isMobile ? '100%' : (hasSelection ? '50%' : '0%'),
          height: isMobile ? (hasSelection ? '55%' : '0%') : '100%',
          opacity: modalVisible ? 1 : 0,
          transition: 'width 500ms cubic-bezier(0.22,1,0.36,1), height 500ms cubic-bezier(0.22,1,0.36,1), opacity 300ms ease',
          overflow: 'hidden',
        }}
      ...
      >
        {selectedItem && (
          <div
            className="w-full h-full flex items-start md:items-center justify-center overflow-y-auto p-4 md:p-6"
            style={{
              transform: modalVisible
                ? 'translateX(0)'
                : (isMobile ? 'translateY(24px)' : 'translateX(24px)'),
              opacity: modalVisible ? 1 : 0,
          ...
          >
            ...
            >
              {/* Close btn */}
              <button
                className="absolute right-3 top-3 z-10 flex h-7 w-7 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/50 transition hover:bg-white/10 hover:text-white"
                onClick={close}
              >
                <X size={13} />
              </button>
              ...
              <CardContent className="max-h-[55vh] space-y-4 overflow-y-auto pb-5">
                {/* Description */}
